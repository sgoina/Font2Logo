<template>
  ...
  <div class="grid card flex justify-content-center">
    <Sidebar v-model:visible="visibleLeft" header="Font2Logo" position="left">
      ...
      <h4>made by rail_hail and segonia</h4>
    </Sidebar>
    ...
    <div class="grid" style="margin-top: 1%">
      <div class="col">
        <Button
          style="height: 40px; width: 40px; font-size: 3rem; padding: 0%; margin-right: 0%"
          icon="pi  pi-cog"
      ...
      <div class="col">
        <Button
          @click="handleSubmit"
          style="width: 130px; height: 40px; margin-left: 0%"
          label="Create Font"
        />
      </div>
      <div class="col">
...
async function updateBar() {
  const res = await fetch_api_att2font('/attributes', false)
  console.log(res.attributes)
  numbersStore.updateNumbers(res.attributes)
}
...
async function switchfont() {
  if (!selectedFontA.value) {
    toast.add({
      severity: 'error',
      summary: 'Error Message',
      detail: 'you need to  select a Font',
      life: 3000
    })
