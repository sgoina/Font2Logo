...
from utils import parse_attributes_file,load_attribute_from_font,get_font_folder_names, update_file_except_png_numbers, get_last_prefix,create_attribute_picture
...
def get_attributes():
    ...
    try:
        # Read the new_attribute.txt file
        with open(NEW_ATTRIBUTES_DIR, 'r') as file:
            attributes_line = file.read().strip()

        # Split the attributes line by spaces
        attribute_values = attributes_line.split(' ')

        # Convert attribute values to floats
        attribute_values = [float(val) for val in attribute_values]

        return jsonify({"attributes": attribute_values}), 200

    except FileNotFoundError:
        return jsonify({"error": "new_attribute.txt file not found"}), 404
...
def update_attributes(prefix):
    ...
    # Update the new_attribute.txt file
    with open(NEW_ATTRIBUTES_DIR, 'w') as file:
        file.write(' '.join(map(str, attribute_values)))
...
def change_image():
    ...
    opts.font_name = files.get('imageA')
    # set font name to image A
    # now load the attribute.txt into the new_attribute.txt with font name
    opts.test_epoch = TEST_EPOCH
    load_attribute_from_font(opts=opts)
