...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size(2) > 1 and x.size(3) > 1:  # For spatial dimensions larger than 1x1
            ...
            out = self.relu(self.norm1d_1(
                self.conv1(x).squeeze(-1).squeeze(-1)))
            out = self.norm1d_2(self.conv2(
                out.view(out.size(0), out.size(1), 1, 1)).squeeze(-1).squeeze(-1))
...
class AttributeAwareUp(nn.Module):
    def __init__(self, in_channel, out_channel, attr_channel, attr_down_scale,
        ...
        img_layers = [nn.ConvTranspose2d(
            in_channel, out_channel, 4, 2, 1, bias=bias)]
        if attention:
...
class StyleEncoder(nn.Module):
    ...
    def forward(self, style, attr_intensity):
        ...
        source_style = source_style.view(
            source_style.size(0), source_style.size(1), 1)
        attr_intensity = attr_intensity.view(
            attr_intensity.size(0),  attr_intensity.size(1), 1)
...
class GeneratorStyle(nn.Module):
    def __init__(self, in_channel=3, n_style=4, attr_channel=37,
        ...
        self.fc_content = Down(512, 62, normalize=False,
                               lrelu=False, kernel_size=1, stride=1, padding=0)
        ...
        self.up3 = AttributeAwareUp(
            1024, 256, attr_channel, 3, attention=attention)
        self.up4 = AttributeAwareUp(
            512, 128, attr_channel, 2, attention=attention)
...
class DiscriminatorWithClassifier(nn.Module):
    ...
    def forward(self, img_A, img_B, charclass, attr_B_intensity):
        if self.pred_attr:
            ...
            attr_B_intensity = attr_B_intensity.view(
                attr_B_intensity.size(0), attr_B_intensity.size(1), 1, 1)
            attr_B_intensity = attr_B_intensity.repeat(
                1, 1, img_A.size(2), img_A.size(3))
...
class CXLoss(nn.Module):
    ...
    def forward(self, featureT, featureI):
        ...
        CX = CX.mean(1)
        # find NaN too large or too small
        CX = -torch.log(torch.clamp(CX, min=1e-8))
        CX = torch.mean(CX)
        return CX
